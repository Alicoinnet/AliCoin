...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("alicoin", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("alicoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("alicoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("alicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("alicoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("alicoin", "Running alicoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
