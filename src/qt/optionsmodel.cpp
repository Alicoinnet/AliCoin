...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::alicoin);
    ...
    if (!settings.contains("nAnonymizealicoinAmount"))
        settings.setValue("nAnonymizealicoinAmount", 1000);

    nAnonymizealicoinAmount = settings.value("nAnonymizealicoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizealicoinAmount"))
        SoftSetArg("-anonymizealicoinamount", settings.value("nAnonymizealicoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in alicoin.cpp during shotdown to also remove the window positions
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizealicoinAmount:
            return QVariant(nAnonymizealicoinAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizealicoinAmount:
            nAnonymizealicoinAmount = value.toInt();
            settings.setValue("nAnonymizealicoinAmount", nAnonymizealicoinAmount);
            emit anonymizealicoinAmountChanged(nAnonymizealicoinAmount);
            break;
        case CoinControlFeatures:
